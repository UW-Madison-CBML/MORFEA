"""
Simplified ConvLSTM Autoencoder
- Uses true ConvLSTM (not regular LSTM)
- Simpler Encoder (2D CNN + ConvLSTM)
- Simpler Decoder (ConvLSTM + ConvTranspose)
- Optional Empty/Non-empty Classifier
- Handles 500x500 input images via interpolation to 256x256
...
class Encoder(nn.Module):
    """
    Encoder: Interpolate to 256x256 + 2D CNN spatial compression + ConvLSTM temporal modeling + Linear compression
    ...
    def __init__(self, input_channels=1, hidden_dim=256, num_layers=2, latent_size=4000):
        ...
        # Spatial convolution layers (no skip connections)
        # After interpolation: 256x256 -> 128x128 -> 64x64 -> 32x32
        ...
        # Input: (B*T, hidden_dim * 32 * 32)
        # Output: (B*T, latent_size)
        self.latent_compress = nn.Linear(hidden_dim * 32 * 32, latent_size)
    ...
    def forward(self, x):
        ...
        # Interpolate from 500x500 to 256x256
        x = x.view(B * T, C, H, W)  # (B*T, 1, 500, 500)
        x = F.interpolate(x, size=(256, 256), mode='bilinear', align_corners=True)  # (B*T, 1, 256, 256)

        # Spatial compression: process each frame separately
        x = self.spatial_cnn(x)  # (B*T, 256, 32, 32)

        _, C2, H2, W2 = x.shape
        x = x.view(B, T, C2, H2, W2)  # (B, T, 256, 32, 32)

        # ConvLSTM processes temporal sequence
        lstm_out, _ = self.convlstm(x)  # list of (B, T, hidden_dim, 32, 32)
        h_seq = lstm_out[0]             # (B, T, hidden_dim, 32, 32)

        # Flatten and compress spatial dimensions with linear layer
        B, T, C, H, W = h_seq.shape
        h_flat = h_seq.view(B * T, C * H * W)  # (B*T, hidden_dim * 32 * 32)
...
class Decoder(nn.Module):
    """
    Decoder: Linear expansion + ConvLSTM temporal decoding + ConvTranspose spatial reconstruction + Interpolate to 500x500
    ...
    def __init__(self, seq_len, latent_size=4000, latent_dim=256, hidden_dim=128, num_layers=2):
        ...
        # Output: (B*T, latent_dim * 32 * 32)
        self.latent_expand = nn.Linear(latent_size, latent_dim * 32 * 32)
        ...
        # Spatial decoding: 32x32 -> 64x64 -> 128x128 -> 256x256
    ...
    def forward(self, z_seq):
        ...
        z_expanded = self.latent_expand(z_flat)  # (B*T, latent_dim * 32 * 32)
        z_spatial = z_expanded.view(B, T, self.latent_dim, 32, 32)  # (B, T, latent_dim, 32, 32)

        # ConvLSTM decodes temporal dimension
        lstm_out, _ = self.convlstm(z_spatial)  # list of (B, T, hidden_dim, 32, 32)
        h_seq = lstm_out[0]                 # (B, T, hidden_dim, 32, 32)

        # Spatial decoding: process each timestep separately
        B, T, C, H, W = h_seq.shape
        h_seq = h_seq.view(B * T, C, H, W)  # (B*T, hidden_dim, 32, 32)
        x_rec = self.spatial_decoder(h_seq)  # (B*T, 1, 256, 256)

        # Interpolate from 256x256 to 500x500
        x_rec = F.interpolate(x_rec, size=(500, 500), mode='bilinear', align_corners=True)  # (B*T, 1, 500, 500)
        x_rec = x_rec.view(B, T, 1, 500, 500)  # (B, T, 1, 500, 500)
