...
for (cell_id, time_step), grouped_df in df.groupby(['cell_id','time_step']):
    first_lat_row = grouped_df.itertples(index=False)[0] # first two entries cell_id and index will always be the same
    for lat_row in grouped_df.itertuples(index=False):
