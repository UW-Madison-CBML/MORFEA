...
=============================================
Changes from Ver09:
    - Temporal loss: CPC -> Rolled InfoNCE
    - infonce_weight default: 0.1
    - batch_size: 32 (kept from Ver09 for longer sequences)

Usage:
    python ver10_train.py convlstm --name my_run
    python ver10_train.py convlstm --infonce-weight 0  # reconstruction only
"""
...
def push_checkpoint(model: torch.nn.Module, repo_name: str, cfg: argparse.Namespace):
    ...
    for fp in ["ver10_train.py", "ver10_model.py", "ver10_losses.py",
               "dataset_ivf.py", "ver10_build_index.py"]:
...
def validate(
    ...
    from losses import rolled_infonce

    stats = {k: RunningStats() for k in [
        "mse", "l1", "ms_ssim_loss", "infonce_loss"
    ]}
    model.eval()
    with torch.no_grad():
        for batch in val_loader:
            ...
            # MS-SSIM requires min side > 160; upsample small images
            smaller_side = min(H, W)
            if smaller_side < 161:
                scale = 161.0 / float(smaller_side)
                new_h, new_w = int(round(H * scale)), int(round(W * scale))
                flat_r = F.interpolate(flat_r, size=(new_h, new_w), mode="bilinear", align_corners=False)
                flat_t = F.interpolate(flat_t, size=(new_h, new_w), mode="bilinear", align_corners=False)
            ms = _ms_ssim(flat_r, flat_t, data_range=1.0, size_average=True)
            stats["ms_ssim_loss"].push((1 - ms).item())

            if B >= 2:
                _, nce_m = rolled_infonce(out["z_seq"],
                                          temperature=cfg.infonce_temp)
                stats["infonce_loss"].push(nce_m["infonce_loss"])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--name", default="convlstm_ver10")
    ...
    g.add_argument("--infonce-weight", type=float, default=0.1,
                   help="Rolled InfoNCE weight (0 to ablate)")
    g.add_argument("--infonce-temp",   type=float, default=0.07)

    g = p.add_argument_group("Architecture")
    g.add_argument("--encoder-dim",    type=int,   default=256)
