...
class PathSigStageDataset(Dataset):
    PHASES = ['pre_phase', 'tPB2', 'tPNa', 'tPNf', 't2', 't3', 't4', 't5', 't6', 't7', 't8', 't9+', 'tM','tSB','tB', 'tEB', 'tHB', 'post_phase']
    def __init__(self, df, time_offsets, pca_dim=8, depth=3)
    """
    ...
    def __get_item__(self, idx):
        ...
        return torch.from_numpy(row[self.ps_cols].to_numpy()), torch.from_numpy(self.__class__.PHASES.index(row["phase"])).to(torch.long)
    def __len__(self):
        return len(self.df)
