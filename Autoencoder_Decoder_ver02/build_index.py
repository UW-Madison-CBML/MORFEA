...
def build_index(root_dir: Path, out_path: Path, seq_len: int, subsample: int):
    ...
    try:
        resolved = root_dir.resolve(strict=True)
        print(f"[build_index] Scanning directory: {root_dir}", flush=True)
        print(f"[build_index] Resolved to: {resolved}", flush=True)
        root_dir = resolved
    except (FileNotFoundError, RuntimeError) as e:
        print(f"[build_index] ERROR: Cannot resolve path '{root_dir}'", flush=True)
        print(f"[build_index] Error: {e}", flush=True)
        if root_dir.is_symlink():
            target = root_dir.readlink()
            print(f"[build_index] Symlink '{root_dir}' points to: {target}", flush=True)
            print(f"[build_index] Target exists: {target.exists() if hasattr(target, 'exists') else 'unknown'}", flush=True)
    ...
    if not root_dir.is_dir():
        ...
        print(f"[build_index] It is: {'file' if root_dir.is_file() else 'symlink' if root_dir.is_symlink() else 'unknown type'}", flush=True)
        sys.exit(1)
